...
public class UsuarioController {
    ...
    public ResponseEntity<?> listarMiembrosPorLider(@PathVariable String matriculaLider, Authentication auth) {
        try {
            String requesterMatricula = (String) auth.getPrincipal();
            boolean isAdmin = auth.getAuthorities().stream()
                    .anyMatch(a -> a.getAuthority().equals("ROLE_ADMINISTRADOR"));

            if (!isAdmin && !requesterMatricula.equals(matriculaLider)) {
                Map<String, String> error = new HashMap<>();
                error.put("error", "No autorizado: Solo puedes consultar los miembros de tu propio equipo");
